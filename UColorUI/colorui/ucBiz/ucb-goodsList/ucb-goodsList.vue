  ...
  * | imgurl     | 商品图片 |String  |   ''    |
  * | desc       | 商品描述 | String |   '' |
  * | taglist    | 商品状态小标签列表 | Array  |  ''     |
  * | buyprice   | 购买价格/现价 | String  |  ''     |
  * | marketprice  | 市场价/原价 | String  |  ''     |
  * | soldcount   | 销售数量 | String  |  ''     |
  *
  * @example <ucb-goodsList :list = "goodlist" mainstyle="double" :iscard="true"></ucb-goodsList>
  */

<template>
	<view>
		<view class="cu-card goods col-goods col-twice" v-if="mainstyle=='double'" :class="iscard==false?'no-card':''">
			<view class="cu-item shadow bg-white" v-for="(item,index) in list" :key="index" @tap="toGoodsDetail()">
		...
		<view class="cu-card goods" v-if="mainstyle=='single'" :class="iscard==false?'no-card':''">
			<view class="cu-item bg-white" v-for="(item,index) in list" :key="index" @tap="toGoodsDetail()">
				<image :src="item.imgurl" mode="aspectFill"></image>
				<view class="content">
					<view class="title">
						<view class="text-line2cut">{{item.desc}}</view>
