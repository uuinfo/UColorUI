  ...
  * | title       | 商品标题 | String |   '' |
  * | buyprice    | 现价 | String  |  ''     |
  * | marketprice   | 原价 | String  |  ''     |
  * | soldpercent  | 已抢购的百分比 | String  |  '' |
  * 
  * @example <ucb-msList :list = "goodlist" :mainstyle="common" :iscard="true"></ucb-msList>
  */
...
<template>
	<view>
		...
		<view class="cu-card goods" v-if="mainstyle=='common'" :class="iscard==false?'no-card':''">
			<view class="cu-item bg-white shadow" v-for="(item,index) in list" :key="index" @tap="toGoodsDetail()">
